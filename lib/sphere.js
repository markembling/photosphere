...
Photosphere.prototype.cropImage = function(){
	...
	img.onload = function(){
		...
		canvas.width = self.exif['crop_width']  //self.exif['full_width'];
		canvas.height = self.exif['crop_height']  //self.exif['full_height'];
		context = canvas.getContext("2d");
	
		context.fillStyle = "#000";
		context.fillRect(0,0,canvas.width,canvas.height);
		context.drawImage(img,  0,0,/*self.exif['x'], self.exif['y'],*/ self.exif['crop_width'], self.exif['crop_height']);
...
Photosphere.prototype.start3D = function(image){
	...
	var thetaOffset = this.exif.y / this.exif.full_width * (Math.PI * 2); //0;
	...
	this.lat = 180 * ((this.exif.crop_height) / 2) / this.exif.full_height - 90; // 180*(x - 0)/2000 - 90
...
Photosphere.prototype.render = function(){
	...
	var maxNorthLatDegrees = ( this.exif['y'] / hemisphereHeight * 90 );
	var maxSouthLatDegrees = ( ( (parseInt(this.exif['y']) + parseInt(this.exif['crop_height'])) - hemisphereHeight ) / hemisphereHeight * 90 );
	maxNorthLatDegrees = Math.min(maxNorthLatDegrees, 85);
	maxSouthLatDegrees = Math.min(maxSouthLatDegrees, 85);
